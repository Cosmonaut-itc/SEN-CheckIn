 ...
 */
class RecognitionBadRequestError extends Error {
...
export const recognitionRoutes = new Elysia({ prefix: '/recognition' })
	...
	.post(
		...
		async ({
			...
			try {
				const parseMeasurement = measureSync(() => {
					const rawBody = JSON.stringify(body);
					const payloadBytes = Buffer.byteLength(rawBody);
					const parsedBody = JSON.parse(rawBody) as unknown;
					const parsedResult = imageBodySchema.safeParse(parsedBody);
			...
			diagnostics.timings.total = performance.now() - requestStartedAt;
