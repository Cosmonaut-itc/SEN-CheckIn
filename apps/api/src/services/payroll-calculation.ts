...
export function calculatePayrollFromData(
	...
	for (const emp of employees) {
		...
		const hasDualPayrollComplement = dualPayrollApplied && (complementPay ?? 0) > 0;
		const netPay = hasDualPayrollComplement
			? grossPay
		...
		const finalNetPay = roundCurrency(Math.max(0, netPay - totalDeductions));
