...
Android dark mode capture was attempted on the `Medium Phone API 36.1` emulator, but Expo Go became blocked by repeated `System UI isn't responding` overlays after the emulator restart. The code paths for Android were still covered by automated tests and quality gates in this branch.
