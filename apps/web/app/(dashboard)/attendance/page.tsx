 ...
 * Provides a list view with date-fns based filtering.
 *
...
export default function AttendancePage(): React.ReactElement {
	...
	/**
	 * Fetches attendance records from the API.
	 */
	const fetchRecords = useCallback(async (): Promise<void> => {
		...
		try {
			...
			const response = await api.attendance.get({
				$query: {
					limit: 100,
					offset: 0,
					fromDate: start,
			...
			if (response.data?.data) {
				setRecords(response.data.data as AttendanceRecord[]);
			}
		} catch (error) {
	...
	}, [datePreset, typeFilter, getDateRange]);
