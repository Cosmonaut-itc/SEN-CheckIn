...
/**
 * Props for the aguinaldo receipts dialog.
 */
type AguinaldoRunReceiptsDialogProps = {
	/** Aguinaldo run metadata. */
	run: AguinaldoRun;
};
...
/**
 * Resolves the net pay displayed for an aguinaldo employee.
 *
 * @param employee - Aguinaldo run employee row
 * @returns Net pay value
 */
function resolveEmployeeNetPay(employee: AguinaldoRunEmployee): number {
	return Number(employee.netAmount ?? 0);
}

/**
 * Aguinaldo receipts dialog for a processed run.
 *
 * @param props - Dialog props
 * @returns Dialog element with receipt downloads
 */
export function AguinaldoRunReceiptsDialog({
	run,
}: AguinaldoRunReceiptsDialogProps): React.ReactElement {
	const t = useTranslations('Aguinaldo');
	const tCommon = useTranslations('Common');
	const [open, setOpen] = useState(false);

	const runDetailQuery = useQuery({
		queryKey: queryKeys.aguinaldo.runDetail(run.id),
		queryFn: () => fetchAguinaldoRunDetail(run.id),
		enabled: open,
	});
	...
	const downloadAllUrl = `/api/aguinaldo/receipts/run/${run.id}/all`;
								...
								<TableBody>
									{isLoading ? (
										...
										employees.map((employee) => {
											...
											const downloadUrl = `/api/aguinaldo/receipts/run/${run.id}/employee/${employee.employeeId}`;
