...
import { buildAguinaldoReceiptFileName } from '@/lib/payroll-receipts/receipt-file-names';
...
/**
 * Generates an aguinaldo receipt PDF for a single employee.
 *
...
export async function GET(
	...
	const resolvedEmployeeCode =
		...
		((employee.employeeId ?? employee.id)
			...
			: '');
	const fileName = buildAguinaldoReceiptFileName({
		employeeCode: resolvedEmployeeCode,
		employeeId: employee.employeeId ?? employee.id,
		calendarYear: detail.run.calendarYear,
	});
