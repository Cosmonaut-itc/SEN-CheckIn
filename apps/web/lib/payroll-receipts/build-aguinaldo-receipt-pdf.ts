...
type AguinaldoReceiptInput = {
	run: AguinaldoRun;
	employee: AguinaldoRunEmployee;
...
/**
 * Builds an Aguinaldo receipt PDF document.
 *
 * @param input - Aguinaldo receipt inputs
 * @returns Serialized PDF bytes
 */
export async function buildAguinaldoReceiptPdf(
	input: AguinaldoReceiptInput,
): Promise<Uint8Array> {
	...
	const grossAmount = toNumber(input.employee.grossAmount);
	...
	page.drawText(input.t('details.period', { value: String(input.run.calendarYear) }), {
		x: detailRightX,
		y: detailRightY,
		size: 10,
		font,
	...
	const incomeLines: ReceiptLine[] = [
		{
			label: input.t('income.lines.aguinaldo'),
			value: grossAmount,
		},
	];
	...
	const receiptMessage = input.t('receiptMessage', {
		...
		period: String(input.run.calendarYear),
	});
